...
for (i in c(1:nrow(FULL_clusters))) {
  ...
  new = data.frame(central_cf = rep(NA, rows), radius_cf = rep(NA, rows))
...
if (!is.na(Verify_Dupes$EXS) & !is.na(Verify_Dupes$EXE)) {
  composites = composite(Verify_Dupes[,3:9])
} else if (!is.na(Verify_Dupes$EXS)) {
  composites = composite(Verify_Dupes[,c("G", "S", "B", "SD", "SE", "EXS")])
} else if (!is.na(Verify_Dupes$EXE)) {
  composites = composite(Verify_Dupes[,3:c("G", "S", "B", "SD", "SE", "EXE")])
} else {
  composites = composite(Verify_Dupes[,3:7])
}
    
Verify_Dupes$match.score = NA
for (i in c(1:nrow(Verify_Dupes))) {
  Verify_Dupes$match.score[i] = composites[i]
}
...
for (o in c(1:nrow(Verify_Dupes))) {
  if(as.integer(Verify_Dupes$central_cf[o]) == as.integer(Verify_Dupes$radius_cf[o])){
    Verify_Dupes = Verify_Dupes[-o,]
  }

}

Verify_Dupes = Verify_Dupes[!duplicated(Verify_Dupes[,c('test')]),]
Verify_Dupes = data.frame(central_cf = Verify_Dupes$central_cf, radius_cf = Verify_Dupes$radius_cf, match.score = Verify_Dupes$match.score)
