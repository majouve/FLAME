...
# disconsidered = read.csv("Not_A_Duplicate.xlsx")
# CoinFinds = CoinFinds[CoinFinds$ID %!in% disconsidered$ID, ]
...
# taking out coin groups where metal == 0
...
Metal Filter: Uses amount of each metal in a coin find and compares this between the central coin find and the radius coin finds. Generates TRUE/FALSE list like above that's added to Cut_CoinFinds. Also generates columns of total amounts of each metal within a coin find. 
```{r}
...
```{r}
FULL_clusters = data.frame(cfID = TEST_CoinFinds$ID, name = TEST_CoinFinds$cf_name, clusterID = TEST_CoinFinds$clusterID)
...
I have added an extra indicator for metals to interpret the date more easily. 
For many coin finds, there will be no coins of a specific metal. For each metal I have added a boolean 
value "is.X.zero" that will be TRUE when all coin finds in a cluster have no coins of X metal (FALSE otherwise)
```{r}
...
for (i in c(1:nrow(FULL_clusters))) {
  ...
  FULL_clusters$avg.coin.num.dif[i] = mean(coin.difs)
}
...
for (i in c(1:nrow(FULL_clusters))) {
  ...
  FULL_clusters$avg.start.date.dif[i] = mean(start.date.difs)
...
for (i in c(1:nrow(FULL_clusters))) {
  ...
  FULL_clusters$avg.end.date.dif[i] = mean(end.date.difs)
}
...
for (i in c(1:nrow(FULL_clusters))) {
  ...
  FULL_clusters$avg.silver.dif[i] = mean(silver.difs)
  FULL_clusters$avg.gold.dif[i] = mean(gold.difs)
  FULL_clusters$avg.bronze.dif[i] = mean(bronze.difs)
  FULL_clusters$avg.lead.dif[i] = mean(lead.difs)
  
  FULL_clusters$is.lead.zero[i] = ifelse(length(unique(lead.difs)) == 1 & unique(lead.difs)[1]==0, TRUE, FALSE)
  FULL_clusters$is.gold.zero[i] = ifelse(length(unique(gold.difs)) == 1 & unique(gold.difs)[1]==0, TRUE, FALSE)
  FULL_clusters$is.silver.zero[i] = ifelse(length(unique(silver.difs)) == 1 & unique(silver.difs)[1]==0, TRUE, FALSE)
  FULL_clusters$is.bronze.zero[i] = ifelse(length(unique(bronze.difs)) == 1 & unique(bronze.difs)[1]==0, TRUE, FALSE)
}

```

Adding one more column to FULL_clusters:
column indicating how many attributes of the coin find clusters match up
```{r}
