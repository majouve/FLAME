...
for (i in c(1:nrow(Verify_Dupes))) {
  ...
  Verify_Dupes$radius_bib[i] = Dupe_CoinFinds$cf_publication_ref[Verify_Dupes$radius_cf[i] == Dupe_CoinFinds$ID]
...
outliers = Verify_Dupes[abs(Verify_Dupes$central_count - Verify_Dupes$radius_count)>300, ]
Verify_Dupes = Verify_Dupes[abs(Verify_Dupes$central_count - Verify_Dupes$radius_count)<300, ]
Verify_Dupes$dupe.id = c(1:nrow(Verify_Dupes))
...
Verify_Dupes$total.dif = abs(Verify_Dupes$central_count - Verify_Dupes$radius_count)
dif.range = c(min(Verify_Dupes$total.dif): max(Verify_Dupes$total.dif))

for (z in c(1:nrow(Verify_Dupes))) {
  percent_dif[z] = mean(dif.range > Verify_Dupes$total.dif[z])
  Verify_Dupes$total.dif[z] = percent_dif[z]
}
...
z_G <- (Verify_Dupes$G - (mean(Verify_Dupes$G)))/(sd(Verify_Dupes$G))
z_B <- (Verify_Dupes$B - (mean(Verify_Dupes$B)))/(sd(Verify_Dupes$B))
z_S <- (Verify_Dupes$S - (mean(Verify_Dupes$S)))/(sd(Verify_Dupes$S))
z_ED <- (Verify_Dupes$ED - (mean(Verify_Dupes$ED)))/(sd(Verify_Dupes$ED))
z_SD <- (Verify_Dupes$SD - (mean(Verify_Dupes$SD)))/(sd(Verify_Dupes$SD))

z_sequ <- (Verify_Dupes$sequential - (mean(Verify_Dupes$sequential)))/(sd(Verify_Dupes$sequential))
z_single <- (Verify_Dupes$singleton - (mean(Verify_Dupes$singleton)))/(sd(Verify_Dupes$singleton))
z_hist <- (Verify_Dupes$distribution - (mean(Verify_Dupes$distribution)))/(sd(Verify_Dupes$distribution))
z_totdif <- (Verify_Dupes$total.dif - (mean(Verify_Dupes$total.dif)))/(sd(Verify_Dupes$total.dif))


# z_VD = data.frame(z_sequ, z_single, z_hist)
# 
# psych::alpha(z_VD, check.keys=TRUE)
...
#   ((-(z_ED[i])))*5
# 
# Verify_Dupes$ms.original[i] = num
##############################
...
# create columns where you tweak equation: with start and end date being increased
# create difference in coin find ammount metric 
...
plot = lm((abs(Verify_Dupes$central_count-Verify_Dupes$radius_count)~Verify_Dupes$ms.A))
print(summary(plot))

plot(Verify_Dupes$ms.A, abs(Verify_Dupes$central_count-Verify_Dupes$radius_count), xlab="match score", ylab="Dif in coin amounts")
abline(lm(abs(Verify_Dupes$central_count-Verify_Dupes$radius_count)~Verify_Dupes$ms.A))

# regress_DF = data.frame(match.score = Verify_Dupes$ms.original, dif = abs(Verify_Dupes$central_count-Verify_Dupes$radius_count))
# 
# outliers <- Verify_Dupes[abs(rstandard(plot)) > 3, ]
# print(outliers)
# 
# for(i in c(1:nrow(Verify_Dupes))) {
#   
#   if (Verify_Dupes$dupe.id[i] %in% outliers$dupe.id){
#     Verify_Dupes$outlier[i] = Verify_Dupes$outlier[i] + 1
#   }
#   
# }
# 
# overall_outliers = Verify_Dupes[Verify_Dupes$outlier!=0,]
```


Which pairs show up the most as residuals when we overweight values?
```{r}
...
```{r}
write_xlsx(Verify_Dupes,"/cloud/project/VerifyDupes.xlsx")
```
