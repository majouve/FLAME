...
Do I try this without zeros?
```{r}
Verify_Dupes$distribution = NA

# 1st for loop that goes through verify_dupes
# It creates two new data frames in for loop and populates them with 325-750)
# Then it uses the coin groups to fill in for each cell proper values. 
  # It would take (coin amount / ((start - end)+1) and then that ammount in each
# then K-S test with p vals. no need to worry about x bc it's constant. 

# create data frame for the histogram with years
...
for (f in c(1:nrow(Verify_Dupes))) {
  ...
  # Histogram = Histogram[Histogram$coins.per.year.cent>0,]
  # Histogram = Histogram[Histogram$coins.per.year.rad>0,]
...
# }
  ...
  if (rad_start_range == 0 & cent_start_range == 0) {
    ...
    Verify_Dupes$S[index] = abs((rad_start_range - cent_start_range)/rad_start_range)
   } else {
  ...
  if (cent_end_range == 0 & rad_end_range == 0) {
    ...
    Verify_Dupes$S[index] = abs((rad_end_range - cent_end_range)/cent_end_range) 
  } else {
...
# for (i in c(1:nrow(Verify_Dupes))) {
#   
#   composites = composite(Verify_Dupes[i, c("G", "S", "B", "SD", "ED")])
#   
